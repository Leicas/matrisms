...
build:
	CGO_CFLAGS="$(CGO_CFLAGS)" CGO_LDFLAGS="$(CGO_LDFLAGS)" go build -ldflags "$(GO_LDFLAGS)" -o matrisms ./cmd/matrisms

clean:
	rm -f matrisms

install: build
	install -m 755 matrisms /usr/local/bin/

...
var (
	SMSConnectionFailed = status.BridgeStateErrorCode("E-SMS-002")
	SMSPollFailed       = status.BridgeStateErrorCode("E-SMS-003")
	SMSCircuitOpen      = status.BridgeStateErrorCode("E-SMS-007")
)
...
func (sc *StateCoordinator) computeBridgeState() (status.BridgeStateEvent, status.BridgeStateErrorCode) {
	...
	if sc.circuitState == "open" {
		return status.StateBridgeUnreachable, SMSCircuitOpen
	}
	...
	if !sc.inbox.Connected {
		...
		return status.StateBridgeUnreachable, SMSConnectionFailed
	}
	...
	if sc.inbox.Connected && !sc.inbox.IdleRunning {
		...
		return status.StateTransientDisconnect, SMSPollFailed
	}
